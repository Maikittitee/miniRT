...
/*   Updated: 2024/01/29 00:41:58 by ktunchar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	init_viewport(t_viewport *viewport, t_cam *cam)
{
	...
	viewport->width = 2 * tan(cam->fov * PI / 360);
	viewport->height = WIN_HEIGHT/WIN_WIDTH * viewport->width;
	...
	viewport->step_x = vector_scaler((float)(1 / WIN_WIDTH) ,viewport->width_vec);
	viewport->step_x = vector_scaler((float)(1 / WIN_HEIGHT) ,viewport->height_vec);
	first_pixel = vector_sub(viewport->origin, vector_scaler(0.5, viewport->width_vec));
	first_pixel = vector_sub(first_pixel, vector_scaler(0.5, viewport->height_vec));
